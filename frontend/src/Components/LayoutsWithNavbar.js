...
export default function LayoutsWithNavbar() {
  const {setMatrixUserId, setGithubUserId, setPatreonUserId} = useContext(GlobalContext);
  ...
  useEffect( () => {
      const fetchCurrentUser = async () => {
        ...
        if(resp.data.matrix_user_id !== null){
          ...
          const patreonUserId = resp.data.patreon_user_id ? resp.data.patreon_user_id : "";
